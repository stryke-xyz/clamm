...
// Interfaces
import {IBulletXV3Factory} from "./v3-core/contracts/interfaces/IBulletXV3Factory.sol";
import {IBulletXV3MintCallback} from "./v3-core/contracts/interfaces/callback/IBulletXV3MintCallback.sol";
import {IBulletXV3Pool} from "./v3-core/contracts/interfaces/IBulletXV3Pool.sol";
...
abstract contract LiquidityManager is IBulletXV3MintCallback {
    ...
    /// @inheritdoc IBulletXV3MintCallback
    function bulletXV3MintCallback(uint256 amount0Owed, uint256 amount1Owed, bytes calldata data) external override {
    ...
    function addLiquidity(AddLiquidityParams memory params)
        public
        returns (uint128 liquidity, uint256 amount0, uint256 amount1, IBulletXV3Pool pool)
    {
        PoolKey memory poolKey = PoolKey({token0: params.token0, token1: params.token1, fee: params.fee});

        pool = IBulletXV3Pool(computeAddress(factory, poolKey));
    ...
    function verifyCallback(address _factory, address tokenA, address tokenB, uint24 fee)
        internal
        view
        returns (IBulletXV3Pool pool)
    {
        return verifyCallback(_factory, getPoolKey(tokenA, tokenB, fee));
    }

    function verifyCallback(address _factory, PoolKey memory poolKey) internal view returns (IBulletXV3Pool pool) {
        pool = IBulletXV3Pool(computeAddress(_factory, poolKey));
