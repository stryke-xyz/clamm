...
import {LiquidityManager} from "../fusionX-v3/LiquidityManager.sol";

/**
 * @title FusionXV3SingleTickLiquidityHandlerV2
 * @author 0xcarrot
 * @dev This is a handler contract for providing liquidity
 * for FusionX V3 Style AMMs. The V2 version supports reserved liquidity and hooks.
 * Do NOT deploy on zkSync, verifyCallback code needs to be updated for zkSync.
 */
contract FusionXV3SingleTickLiquidityHandlerV2 is
    ERC6909,
    IHandler,
    Pausable,
    AccessControl,
    LiquidityManager
{
    ...
    // errors
    error FusionXV3SingleTickLiquidityHandlerV2__NotWhitelisted();
    error FusionXV3SingleTickLiquidityHandlerV2__InRangeLP();
    error FusionXV3SingleTickLiquidityHandlerV2__InsufficientLiquidity();
    error FusionXV3SingleTickLiquidityHandlerV2__BeforeReserveCooldown();
    ...
    function mintPositionHandler(
        ...
        if (posCache.amount0 > 0 && posCache.amount1 > 0)
            revert FusionXV3SingleTickLiquidityHandlerV2__InRangeLP();
        ...
        if (tki.totalSupply > 0) {
            ...
            if (tki.tokensOwed0 > 1_000 || tki.tokensOwed1 > 1_000) {
                ...
                if (
                    ...
                    ) = FusionXV3SingleTickLiquidityHandlerV2(address(this))
                            .addLiquidity(
                                LiquidityManager.AddLiquidityParams({
                                    token0: tki.token0,
                                    token1: tki.token1,
    ...
    function burnPositionHandler(
        ...
        if ((tki.totalLiquidity - tki.liquidityUsed) < posCache.liquidityToBurn)
            revert FusionXV3SingleTickLiquidityHandlerV2__InsufficientLiquidity();
     ...
     * be burned and they will receive FusionX V3 fees upto this point.
     * @return The number of shares burned.
     */
    ...
    function withdrawReserveLiquidity(
        ...
        if (rld.lastReserve + reserveCooldown > block.timestamp)
            revert FusionXV3SingleTickLiquidityHandlerV2__BeforeReserveCooldown();
    ...
    function getHandlerIdentifier(
        ...
        (
            ...
            int24 tickLower,
            int24 tickUpper
        ) = abi.decode(_data, (IFusionXV3Pool, address, int24, int24));
    ...
    /**
     * @notice Gets the current sqrtPriceX96 of the given FusionXV3Pool.
     * @param pool The FusionXV3Pool to get the sqrtPriceX96 from.
     * @return sqrtPriceX96 The current sqrtPriceX96 of the given FusionXV3Pool.
     */
    ...
    function onlyWhitelisted() private {
        if (!whitelistedApps[msg.sender])
            revert FusionXV3SingleTickLiquidityHandlerV2__NotWhitelisted();
    }
    ...
    /**
     * @notice Forcefully withdraws FusionXV3 liquidity from the given position.
     * @param pool The FusionXV3Pool to withdraw liquidity from.
     ...
     */
    function forceWithdrawFusionXV3Liquidity(
        IFusionXV3Pool pool,
        int24 tickLower,
        int24 tickUpper,
        uint128 liquidity,
