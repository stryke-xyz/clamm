...
import {LiquidityManager} from "../kodiak-v3/LiquidityManager.sol";

/**
 * @title KodiakV3SingleTickLiquidityHandlerV2
 ...
 */
contract KodiakV3SingleTickLiquidityHandlerV3 is ERC6909, IHandler, Pausable, AccessControl, LiquidityManager {
    ...
    // errors
    error KodiakV3SingleTickLiquidityHandlerV3__NotWhitelisted();
    error KodiakV3SingleTickLiquidityHandlerV3__InRangeLP();
    error KodiakV3SingleTickLiquidityHandlerV3__InsufficientLiquidity();
    error KodiakV3SingleTickLiquidityHandlerV3__BeforeReserveCooldown();
    error KodiakV3SingleTickLiquidityHandlerV3__NotWhitelistedPool();
        ...
        returns (uint256 sharesMinted)
    {
        ...
        if (!whitelistedPools[address(_params.pool)]) revert KodiakV3SingleTickLiquidityHandlerV3__NotWhitelistedPool();
        ...
        if (posCache.amount0 > 0 && posCache.amount1 > 0) {
            revert KodiakV3SingleTickLiquidityHandlerV3__InRangeLP();
        }
        ...
        returns (uint256)
    {
        ...
        if ((tki.totalLiquidity - tki.liquidityUsed) < posCache.liquidityToBurn) {
            revert KodiakV3SingleTickLiquidityHandlerV3__InsufficientLiquidity();
        }
    ...
    function withdrawReserveLiquidity(address context, bytes calldata _reserveLiquidityParam) external whenNotPaused {
        ...
        if (rld.lastReserve + reserveCooldown > block.timestamp) {
            revert KodiakV3SingleTickLiquidityHandlerV3__BeforeReserveCooldown();
        }
    ...
    function onlyWhitelisted() private view {
        if (!whitelistedApps[msg.sender]) {
            revert KodiakV3SingleTickLiquidityHandlerV3__NotWhitelisted();
        }
    }
