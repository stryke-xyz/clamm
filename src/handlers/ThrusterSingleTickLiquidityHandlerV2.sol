...
import {LiquidityManager} from "../thruster-v3/LiquidityManager.sol";

contract ThrusterSingleTickLiquidityHandlerV2 is ERC6909, IHandler, Pausable, AccessControl, LiquidityManager {
    ...
    event LogFeeCompound(
        address handler, IThrusterPool pool, uint256 tokenId, int24 tickLower, int24 tickUpper, uint128 liquidity
    );
    ...
    // errors
    error ThrusterSingleTickLiquidityHandlerV2__NotWhitelisted();
    error ThrusterSingleTickLiquidityHandlerV2__InRangeLP();
    error ThrusterSingleTickLiquidityHandlerV2__InsufficientLiquidity();
    error ThrusterSingleTickLiquidityHandlerV2__BeforeReserveCooldown();
        ...
        returns (uint256 sharesMinted)
    {
        ...
        if (posCache.amount0 > 0 && posCache.amount1 > 0) {
            revert ThrusterSingleTickLiquidityHandlerV2__InRangeLP();
        }
        ...
        if (tki.totalSupply > 0) {
            ...
            if (tki.tokensOwed0 > 1_000 || tki.tokensOwed1 > 1_000) {
                ...
                if (expectedAmountForLiquidity0 > tki.tokensOwed0 || expectedAmountForLiquidity1 > tki.tokensOwed1) {
                    ...
                    (uint128 liquidityFee,,,) = ThrusterSingleTickLiquidityHandlerV2(address(this)).addLiquidity(
                        LiquidityManager.AddLiquidityParams({
                            token0: tki.token0,
                            token1: tki.token1,
                            fee: tki.fee,
        ...
        returns (uint256)
    {
        ...
        if ((tki.totalLiquidity - tki.liquidityUsed) < posCache.liquidityToBurn) {
            revert ThrusterSingleTickLiquidityHandlerV2__InsufficientLiquidity();
        }
     ...
     * be burned and they will receive Thruster fees upto this point.
     * @return The number of shares burned.
     */
    ...
    function withdrawReserveLiquidity(bytes calldata _reserveLiquidityParam) external whenNotPaused {
        ...
        if (rld.lastReserve + reserveCooldown > block.timestamp) {
            revert ThrusterSingleTickLiquidityHandlerV2__BeforeReserveCooldown();
        }
     ...
     * @param _pool The ThrusterPool contract.
     * @param _tickLower The lower tick of the position.
     * @param _tickUpper The upper tick of the position.
     */
    function _feeCalculation(TokenIdInfo storage _tki, IThrusterPool _pool, int24 _tickLower, int24 _tickUpper)
        internal
    {
    ...
    function getHandlerIdentifier(bytes calldata _data) external view returns (uint256 handlerIdentifierId) {
        (IThrusterPool pool, address hook, int24 tickLower, int24 tickUpper) =
            abi.decode(_data, (IThrusterPool, address, int24, int24));
    ...
    /**
     * @notice Gets the current sqrtPriceX96 of the given ThrusterPool.
     * @param pool The ThrusterPool to get the sqrtPriceX96 from.
     * @return sqrtPriceX96 The current sqrtPriceX96 of the given ThrusterPool.
     */
    function _getCurrentSqrtPriceX96(IThrusterPool pool) internal view returns (uint160 sqrtPriceX96) {
    ...
    function onlyWhitelisted() private {
        if (!whitelistedApps[msg.sender]) {
            revert ThrusterSingleTickLiquidityHandlerV2__NotWhitelisted();
        }
    }
    ...
    /**
     * @notice Forcefully withdraws Thruster liquidity from the given position.
     * @param pool The ThrusterPool to withdraw liquidity from.
     ...
     */
    function forceWithdrawThrusterLiquidity(
        IThrusterPool pool,
        int24 tickLower,
        int24 tickUpper,
        uint128 liquidity,
