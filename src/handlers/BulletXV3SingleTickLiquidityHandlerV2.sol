...
import {LiquidityManager} from "../bulletX-v3/LiquidityManager.sol";

/**
 * @title BulletXV3SingleTickLiquidityHandlerV2
 * @author 0xcarrot
 * @dev This is a handler contract for providing liquidity
 * for BulletX V3 Style AMMs. The V2 version supports reserved liquidity and hooks.
 * Do NOT deploy on zkSync, verifyCallback code needs to be updated for zkSync.
 */
contract BulletXV3SingleTickLiquidityHandlerV2 is ERC6909, IHandler, Pausable, AccessControl, LiquidityManager {
    ...
    event LogFeeCompound(
        address handler, IBulletXV3Pool pool, uint256 tokenId, int24 tickLower, int24 tickUpper, uint128 liquidity
    );
    ...
    // errors
    error BulletXV3SingleTickLiquidityHandlerV2__NotWhitelisted();
    error BulletXV3SingleTickLiquidityHandlerV2__InRangeLP();
    error BulletXV3SingleTickLiquidityHandlerV2__InsufficientLiquidity();
    error BulletXV3SingleTickLiquidityHandlerV2__BeforeReserveCooldown();
        ...
        returns (uint256 sharesMinted)
    {
        ...
        if (posCache.amount0 > 0 && posCache.amount1 > 0) {
            revert BulletXV3SingleTickLiquidityHandlerV2__InRangeLP();
        }
        ...
        if (tki.totalSupply > 0) {
            ...
            if (tki.tokensOwed0 > 1_000 || tki.tokensOwed1 > 1_000) {
                ...
                if (expectedAmountForLiquidity0 > tki.tokensOwed0 || expectedAmountForLiquidity1 > tki.tokensOwed1) {
                    ...
                    (uint128 liquidityFee,,,) = BulletXV3SingleTickLiquidityHandlerV2(address(this)).addLiquidity(
                        LiquidityManager.AddLiquidityParams({
                            token0: tki.token0,
                            token1: tki.token1,
                            fee: tki.fee,
        ...
        returns (uint256)
    {
        ...
        if ((tki.totalLiquidity - tki.liquidityUsed) < posCache.liquidityToBurn) {
            revert BulletXV3SingleTickLiquidityHandlerV2__InsufficientLiquidity();
        }
     ...
     * be burned and they will receive BulletX V3 fees upto this point.
     * @return The number of shares burned.
     */
    ...
    function withdrawReserveLiquidity(bytes calldata _reserveLiquidityParam) external whenNotPaused {
        ...
        if (rld.lastReserve + reserveCooldown > block.timestamp) {
            revert BulletXV3SingleTickLiquidityHandlerV2__BeforeReserveCooldown();
        }
     ...
     * @param _pool The BulletXV3Pool contract.
     * @param _tickLower The lower tick of the position.
     * @param _tickUpper The upper tick of the position.
     */
    function _feeCalculation(TokenIdInfo storage _tki, IBulletXV3Pool _pool, int24 _tickLower, int24 _tickUpper)
        internal
    {
    ...
    function getHandlerIdentifier(bytes calldata _data) external view returns (uint256 handlerIdentifierId) {
        (IBulletXV3Pool pool, address hook, int24 tickLower, int24 tickUpper) =
            abi.decode(_data, (IBulletXV3Pool, address, int24, int24));
    ...
    /**
     * @notice Gets the current sqrtPriceX96 of the given BulletXV3Pool.
     * @param pool The BulletXV3Pool to get the sqrtPriceX96 from.
     * @return sqrtPriceX96 The current sqrtPriceX96 of the given BulletXV3Pool.
     */
    function _getCurrentSqrtPriceX96(IBulletXV3Pool pool) internal view returns (uint160 sqrtPriceX96) {
    ...
    function onlyWhitelisted() private {
        if (!whitelistedApps[msg.sender]) {
            revert BulletXV3SingleTickLiquidityHandlerV2__NotWhitelisted();
        }
    }
    ...
    /**
     * @notice Forcefully withdraws BulletXV3 liquidity from the given position.
     * @param pool The BulletXV3Pool to withdraw liquidity from.
     ...
     */
    function forceWithdrawBulletXV3Liquidity(
        IBulletXV3Pool pool,
        int24 tickLower,
        int24 tickUpper,
        uint128 liquidity,
