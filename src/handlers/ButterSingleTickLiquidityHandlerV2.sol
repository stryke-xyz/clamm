...
import {LiquidityManager} from "../butter-v3/LiquidityManager.sol";

/**
 * @title ButterSingleTickLiquidityHandlerV2
 * @author 0xcarrot
 * @dev This is a handler contract for providing liquidity
 * for Butter V3 Style AMMs. The V2 version supports reserved liquidity and hooks.
 * Do NOT deploy on zkSync, verifyCallback code needs to be updated for zkSync.
 */
contract ButterSingleTickLiquidityHandlerV2 is ERC6909, IHandler, Pausable, AccessControl, LiquidityManager {
    ...
    event LogFeeCompound(
        address handler, IButterPool pool, uint256 tokenId, int24 tickLower, int24 tickUpper, uint128 liquidity
    );
    ...
    // errors
    error ButterSingleTickLiquidityHandlerV2__NotWhitelisted();
    error ButterSingleTickLiquidityHandlerV2__InRangeLP();
    error ButterSingleTickLiquidityHandlerV2__InsufficientLiquidity();
    error ButterSingleTickLiquidityHandlerV2__BeforeReserveCooldown();
        ...
        returns (uint256 sharesMinted)
    {
        ...
        if (posCache.amount0 > 0 && posCache.amount1 > 0) {
            revert ButterSingleTickLiquidityHandlerV2__InRangeLP();
        }
        ...
        if (tki.totalSupply > 0) {
            ...
            if (tki.tokensOwed0 > 1_000 || tki.tokensOwed1 > 1_000) {
                ...
                if (expectedAmountForLiquidity0 > tki.tokensOwed0 || expectedAmountForLiquidity1 > tki.tokensOwed1) {
                    ...
                    (uint128 liquidityFee,,,) = ButterSingleTickLiquidityHandlerV2(address(this)).addLiquidity(
                        LiquidityManager.AddLiquidityParams({
                            token0: tki.token0,
                            token1: tki.token1,
                            fee: tki.fee,
        ...
        returns (uint256)
    {
        ...
        if ((tki.totalLiquidity - tki.liquidityUsed) < posCache.liquidityToBurn) {
            revert ButterSingleTickLiquidityHandlerV2__InsufficientLiquidity();
        }
     ...
     * be burned and they will receive Butter V3 fees upto this point.
     * @return The number of shares burned.
     */
    ...
    function withdrawReserveLiquidity(bytes calldata _reserveLiquidityParam) external whenNotPaused {
        ...
        if (rld.lastReserve + reserveCooldown > block.timestamp) {
            revert ButterSingleTickLiquidityHandlerV2__BeforeReserveCooldown();
        }
     ...
     * @param _pool The ButterPool contract.
     * @param _tickLower The lower tick of the position.
     * @param _tickUpper The upper tick of the position.
     */
    function _feeCalculation(TokenIdInfo storage _tki, IButterPool _pool, int24 _tickLower, int24 _tickUpper)
        internal
    {
    ...
    function getHandlerIdentifier(bytes calldata _data) external view returns (uint256 handlerIdentifierId) {
        (IButterPool pool, address hook, int24 tickLower, int24 tickUpper) =
            abi.decode(_data, (IButterPool, address, int24, int24));
    ...
    /**
     * @notice Gets the current sqrtPriceX96 of the given ButterPool.
     * @param pool The ButterPool to get the sqrtPriceX96 from.
     * @return sqrtPriceX96 The current sqrtPriceX96 of the given ButterPool.
     */
    function _getCurrentSqrtPriceX96(IButterPool pool) internal view returns (uint160 sqrtPriceX96) {
    ...
    function onlyWhitelisted() private {
        if (!whitelistedApps[msg.sender]) {
            revert ButterSingleTickLiquidityHandlerV2__NotWhitelisted();
        }
    }
    ...
    /**
     * @notice Forcefully withdraws Butter liquidity from the given position.
     * @param pool The ButterPool to withdraw liquidity from.
     ...
     */
    function forceWithdrawButterLiquidityAndToken(
        IButterPool pool,
        int24 tickLower,
        int24 tickUpper,
        uint128 liquidity,
