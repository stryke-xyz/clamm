...
import {LiquidityManager} from "../agni-v3/LiquidityManager.sol";

/**
 * @title AgniSingleTickLiquidityHandlerV2
 * @author 0xcarrot
 * @dev This is a handler contract for providing liquidity
 * for Agni Style AMMs. The V2 version supports reserved liquidity and hooks.
 * Do NOT deploy on zkSync, verifyCallback code needs to be updated for zkSync.
 */
contract AgniSingleTickLiquidityHandlerV2 is
    ERC6909,
    IHandler,
    Pausable,
    AccessControl,
    LiquidityManager
{
    ...
    // errors
    error AgniSingleTickLiquidityHandlerV2__NotWhitelisted();
    error AgniSingleTickLiquidityHandlerV2__InRangeLP();
    error AgniSingleTickLiquidityHandlerV2__InsufficientLiquidity();
    error AgniSingleTickLiquidityHandlerV2__BeforeReserveCooldown();
    ...
    function mintPositionHandler(
        ...
        if (posCache.amount0 > 0 && posCache.amount1 > 0)
            revert AgniSingleTickLiquidityHandlerV2__InRangeLP();
        ...
        if (tki.totalSupply > 0) {
            ...
            if (tki.tokensOwed0 > 1_000 || tki.tokensOwed1 > 1_000) {
                ...
                if (
                    ...
                    ) = AgniSingleTickLiquidityHandlerV2(address(this))
                            .addLiquidity(
                                LiquidityManager.AddLiquidityParams({
                                    token0: tki.token0,
                                    token1: tki.token1,
    ...
    function burnPositionHandler(
        ...
        if ((tki.totalLiquidity - tki.liquidityUsed) < posCache.liquidityToBurn)
            revert AgniSingleTickLiquidityHandlerV2__InsufficientLiquidity();
     ...
     * be burned and they will receive Agni fees upto this point.
     * @return The number of shares burned.
     */
    ...
    function withdrawReserveLiquidity(
        ...
        if (rld.lastReserve + reserveCooldown > block.timestamp)
            revert AgniSingleTickLiquidityHandlerV2__BeforeReserveCooldown();
     ...
     * @param _pool The AgniPool contract.
    ...
    function getHandlerIdentifier(
        ...
        (
            ...
            int24 tickLower,
            int24 tickUpper
        ) = abi.decode(_data, (IAgniPool, address, int24, int24));
    ...
    /**
     * @notice Gets the current sqrtPriceX96 of the given AgniPool.
     * @param pool The AgniPool to get the sqrtPriceX96 from.
     * @return sqrtPriceX96 The current sqrtPriceX96 of the given AgniPool.
     */
    ...
    function onlyWhitelisted() private {
        if (!whitelistedApps[msg.sender])
            revert AgniSingleTickLiquidityHandlerV2__NotWhitelisted();
    }
    ...
    /**
     * @notice Forcefully withdraws Agni liquidity from the given position.
     * @param pool The AgniPool to withdraw liquidity from.
     ...
     */
    function forceWithdrawAgniLiquidity(
        IAgniPool pool,
        int24 tickLower,
        int24 tickUpper,
        uint128 liquidity,
