...
import {LiquidityManager} from "../pancake-v3/LiquidityManager.sol";

contract PancakeV3SingleTickLiquidityHandlerV3 is ERC6909, IHandler, Pausable, AccessControl, LiquidityManager {
    ...
    event LogFeeCompound(
        address handler, IPancakeV3Pool pool, uint256 tokenId, int24 tickLower, int24 tickUpper, uint128 liquidity
    );
    ...
    function getAmountsForLiquidity(IPancakeV3Pool pool, int24 tickLower, int24 tickUpper, uint128 liquidityToDonate)
        public
        view
     ...
     */
    function _feeCalculation(TokenIdInfo storage _tki, IPancakeV3Pool _pool, int24 _tickLower, int24 _tickUpper)
        internal
    {
    ...
    function getHandlerIdentifier(bytes calldata _data) public view returns (uint256 handlerIdentifierId) {
        (IPancakeV3Pool pool, address hook, int24 tickLower, int24 tickUpper) =
            abi.decode(_data, (IPancakeV3Pool, address, int24, int24));

        return _getHandlerIdentifier(pool, hook, tickLower, tickUpper);
    }

    function _getHandlerIdentifier(IPancakeV3Pool pool, address hook, int24 tickLower, int24 tickUpper)
        private
        view
        returns (uint256)
    {
    ...
    /**
     * @notice Gets the current sqrtPriceX96 of the given PancakeV3Pool.
     * @param pool The PancakeV3Pool to get the sqrtPriceX96 from.
     * @return sqrtPriceX96 The current sqrtPriceX96 of the given PancakeV3Pool.
     */
    function _getCurrentSqrtPriceX96(IPancakeV3Pool pool) internal view returns (uint160 sqrtPriceX96) {
     ...
     */
    function forceWithdrawPancakeV3LiquidityAndToken(
        IPancakeV3Pool pool,
        int24 tickLower,
        int24 tickUpper,
        uint128 liquidity,
