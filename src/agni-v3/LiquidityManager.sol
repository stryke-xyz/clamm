...
// Interfaces
import {IAgniFactory} from "./v3-core/contracts/interfaces/IAgniFactory.sol";
import {IAgniMintCallback} from "./v3-core/contracts/interfaces/callback/IAgniMintCallback.sol";
import {IAgniPool} from "./v3-core/contracts/interfaces/IAgniPool.sol";
...
abstract contract LiquidityManager is IAgniMintCallback {
    ...
    function agniMintCallback(uint256 amount0Owed, uint256 amount1Owed, bytes calldata data) external override {
    ...
    function addLiquidity(AddLiquidityParams memory params)
        public
        returns (uint128 liquidity, uint256 amount0, uint256 amount1, IAgniPool pool)
    {
        PoolKey memory poolKey = PoolKey({token0: params.token0, token1: params.token1, fee: params.fee});

        pool = IAgniPool(computeAddress(factory, poolKey));
    ...
    function verifyCallback(address _factory, PoolKey memory poolKey) internal view returns (IAgniPool pool) {
        pool = IAgniPool(computeAddress(_factory, poolKey));
