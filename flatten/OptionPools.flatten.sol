    ...
    Multicall
{
    ...
    event UpdateWhitelist(address _app, bool _status);

    modifier onlyWhitelisted() {
        require(whitelistedApps[msg.sender], "!whitelisted");
        _;
    }
        ...
        returns (uint256 amount0, uint256 amount1, uint256 liquidity)
    {
        uint256 tokenId = uint256(
            keccak256(
                abi.encode(_params.pool, _params.tickLower, _params.tickUpper)
            )
        );
        ...
        if (tki.totalSupply > 0) {
            ...
            if (tki.tokensOwed0 > 0 || tki.tokensOwed1 > 0) {
                ...
                if (amount0 > 0) {
                    ERC20(_params.pool.token1()).approve(
                        address(swapRouter),
                        a1
                    );
                    uint256 amountOut = swapRouter.exactInputSingle(
                        ISwapRouter.ExactInputSingleParams({
                            tokenIn: _params.pool.token1(),
                            tokenOut: _params.pool.token0(),
                            fee: _params.pool.fee(),
                            recipient: address(this),
                    ...
                    (uint256 l0, , , ) = PositionManager(address(this))
                        .addLiquidity(
                            LiquidityManager.AddLiquidityParams({
                                token0: _params.pool.token0(),
                                token1: _params.pool.token1(),
                                ...
                                tickLower: _params.tickLower,
                                tickUpper: _params.tickUpper,
                                amount0Desired: a0 + amountOut,
                                amount1Desired: 0,
                                amount0Min: a0 + amountOut,
                                amount1Min: 0
                            })
                        );
                    tki.totalLiquidity += l0;
                    ...
                    ERC20(_params.pool.token0()).approve(
                        address(swapRouter),
                        a0
                    );
                    uint256 amountOut = swapRouter.exactInputSingle(
                        ISwapRouter.ExactInputSingleParams({
                            tokenIn: _params.pool.token0(),
                            tokenOut: _params.pool.token1(),
                            fee: _params.pool.fee(),
                            recipient: address(this),
                            deadline: block.timestamp + 5 days,
                            amountIn: a0,
                    ...
                    (uint256 l1, , , ) = PositionManager(address(this))
                        .addLiquidity(
                            LiquidityManager.AddLiquidityParams({
                                token0: _params.pool.token0(),
                                token1: _params.pool.token1(),
                                ...
                                tickUpper: _params.tickUpper,
                                amount0Desired: 0,
                                amount1Desired: a1 + amountOut,
                                amount0Min: 0,
                                amount1Min: a1 + amountOut
                            })
                        );
                    tki.totalLiquidity += l1;
                    emit FeeCompound(
                        _params.pool,
                        _params.tickLower,
                        _params.tickUpper,
            ...
            _mint(
                ...
                shares,
                abi.encodePacked(
                    ERC20(_params.pool.token0()).symbol(),
                    "/",
                    ERC20(_params.pool.token1()).symbol(),
                    "-",
                    _params.tickLower,
                    "-",
                    _params.tickUpper
    ...
    function burnPosition(
        BurnPositionParams memory _params
    )
        public
        nonReentrant
        returns (uint256 amount0, uint256 amount1, uint256 liquidityToBurn)
    {
        uint256 tokenId = uint256(
            keccak256(
                abi.encode(_params.pool, _params.tickLower, _params.tickUpper)
        ...
        require(balanceOf(msg.sender, tokenId) >= _params.shares);

        liquidityToBurn = _convertToAssets(
            _params.shares,
            tokenId,
            Math.Rounding.Up
        );

        (amount0, amount1) = _params.pool.burn(
            _params.tickLower,
            _params.tickUpper,
    ...
    function usePosition(
        UsePositionParams memory _params
    )
        public
        onlyWhitelisted
        nonReentrant
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        emit UsePosition(
            ...
            _params.tickLower,
            _params.tickUpper,
            _params.liquidityToUse
        );
    }

    function unusePosition(
        UnusePositionParams memory _params
    )
        public
        onlyWhitelisted
        nonReentrant
    ...
    // admin
    function updateWhitelist(address _app, bool _status) public onlyOwner {
        whitelistedApps[_app] = _status;

        emit UpdateWhitelist(_app, _status);
    }

    function sweepFee(
        SweepFeeParams memory _params
        ...
        (uint256 amount0Collect, uint256 amount1Collect) = (
            _params.amount0Max > tokensOwed0 ? tokensOwed0 : _params.amount0Max,
            _params.amount1Max > tokensOwed1 ? tokensOwed1 : _params.amount1Max
        );

        (amount0, amount1) = _params.pool.collect(
            owner(),
            _params.tickLower,
            _params.tickUpper,
        ...
        (tki.tokensOwed0, tki.tokensOwed1) = (
    ...
    function _feeCalculation(
        ...
        if (feeGrowthInside0LastX128 == 0 && feeGrowthInside1LastX128 == 0) {
    ...
    function _computeFeesEarned(
        ...
        uint256 tokenId = uint256(
            keccak256(abi.encode(pool, tickLower, tickUpper))
        );
        ...
        if (isZero) {
            ...
            (, , feeGrowthOutsideLower, , , , , ) = pool.ticks(tickLower);
            (, , feeGrowthOutsideUpper, , , , , ) = pool.ticks(tickUpper);
        } else {
        ...
        unchecked {
            ...
            fee = FullMath.mulDiv(
                ...
                feeGrowthInside -
                    (
                        isZero
                            ? tki.feeGrowthInside0LastX128
                            : tki.feeGrowthInside1LastX128
                    ),
                0x100000000000000000000000000000000
            );
        }
...
contract OptionPools is ERC1155("Dopex V2 Option Pools") {
    using TickMath for int24;

    IVolatilityEngine volatilityEngine;
    IOptionPricing optionPricing;
    PositionManager positionManager;
    SwapRouter swapRouter;

    bytes32 public volatilityEngineId;

    address public callAsset;
    address public putAsset;
    address owner;

    uint256 MIN_TTL = 20 minutes;
    uint256 MAX_TTL = 24 hours;
    uint256 MIN_PREMIUM = 10_000;

    bool useMinPremium;

    struct MintOptionRollParams {
        IUniswapV3Pool pool;
        int24 tickLower;
        int24 tickUpper;
        uint256 liquidityToUse;
    ...
    modifier validTtl(uint256 ttl) {
        require(MIN_TTL <= ttl && ttl <= MAX_TTL);
        _;
    }
    ...
    constructor(
        ...
        address _volatilityEngine,
        address _callAsset,
        address _putAsset,
        bytes32 _volatilityEngineId
    ) {
        positionManager = _pm;
        swapRouter = _swapRouter;
        ...
        volatilityEngine = IVolatilityEngine(_volatilityEngine);
        volatilityEngineId = _volatilityEngineId;

        owner = msg.sender;
    }

    function mintCallOptionRoll(
        MintOptionRollParams memory _params
    ) external validTtl(_params.ttl) {
        uint256 tokenId = uint256(
            keccak256(
                abi.encode(_params.pool, _params.tickLower, _params.tickUpper)
            )
        ...
        (uint256 amount0, uint256 amount1) = positionManager.usePosition(
            PositionManager.UsePositionParams({
                pool: _params.pool,
                tickLower: _params.tickLower,
                tickUpper: _params.tickUpper,
                liquidityToUse: _params.liquidityToUse
            })
        );

        if (_params.pool.token0() == callAsset) {
            require(amount0 > 0 && amount1 == 0);
        } else {
            require(amount1 > 0 && amount0 == 0);
        }

        uint256 premiumAmount = _getPremiumAmount(
            false, // isPut
            block.timestamp + _params.ttl, // expiry
            getPricePerCallAssetViaTick(_params.pool, _params.tickUpper), // Strike
            getCurrentPricePerCallAsset(_params.pool), // Current price
            volatilityEngine.getVolatility(volatilityEngineId, 0, 0), // IV, strike and expiry param is 0 since we are using flat volatility
            amount0 > 0 ? amount0 : amount1
        );

        ERC20(callAsset).transferFrom(msg.sender, address(this), premiumAmount);

        ERC20(callAsset).approve(address(positionManager), premiumAmount);

        uint128 l = LiquidityAmounts.getLiquidityForAmounts(
            _getCurrentSqrtPriceX96(_params.pool),
            _params.tickLower.getSqrtRatioAtTick(),
            _params.tickUpper.getSqrtRatioAtTick(),
            amount0 > 0 ? premiumAmount : 0,
            amount1 > 0 ? premiumAmount : 0
        );

        (, , uint256 liquidity) = positionManager.unusePosition(
            PositionManager.UnusePositionParams({
                pool: _params.pool,
                tickLower: _params.tickLower,
                tickUpper: _params.tickUpper,
                liquidityToUnuse: l
        ...
        // require(liquidity >= optionInfo.premium);

        uint256 optionTokenId = uint256(
            keccak256(abi.encode(tokenId, block.timestamp + _params.ttl, true))
        );

        emit MintCallOptionRoll(
            _params.pool,
            _params.tickLower,
            _params.tickUpper,
            block.timestamp + _params.ttl,
            premiumAmount,
            _params.liquidityToUse,
            msg.sender
        );

        _mint(msg.sender, optionTokenId, _params.liquidityToUse, "");
    }

    function mintPutOptionRoll(
        MintOptionRollParams memory _params
    ) external validTtl(_params.ttl) {
        uint256 tokenId = uint256(
            keccak256(
        ...
        if (_params.pool.token0() == putAsset) {
            require(amount0 > 0 && amount1 == 0);
        } else {
            require(amount1 > 0 && amount0 == 0);
        }
        ...
        uint256 premiumAmount = _getPremiumAmount(
            true, // isPut
            block.timestamp + _params.ttl, // expiry
            getPricePerCallAssetViaTick(_params.pool, _params.tickLower), // Strike
            getCurrentPricePerCallAsset(_params.pool), // Current price
            volatilityEngine.getVolatility(volatilityEngineId, 0, 0), // IV, strike and expiry param is 0 since we are using flat volatility
            amount0 > 0 ? amount0 : amount1
        );
        ERC20(putAsset).transferFrom(msg.sender, address(this), premiumAmount);

        ERC20(putAsset).approve(address(positionManager), premiumAmount);
        ...
        uint256 optionTokenId = uint256(
            keccak256(abi.encode(tokenId, block.timestamp + _params.ttl, false))
        );

        emit MintPutOptionRoll(
            _params.pool,
            _params.tickLower,
            _params.tickUpper,
            block.timestamp + _params.ttl,
    ...
    function exerciseOptionRoll(
        ExerciseOptionRollParams memory _params
    ) public {
        uint256 tokenId = uint256(
            keccak256(
                abi.encode(_params.pool, _params.tickLower, _params.tickUpper)
        ...
        uint256 optionTokenId = uint256(
            keccak256(abi.encode(tokenId, _params.expiry, _params.callOrPut))
        );

        require(_params.expiry >= block.timestamp);
        require(
            balanceOf(msg.sender, optionTokenId) >= _params.amountToExercise
        );

        if (_params.callOrPut) {
            bool isAmount0 = _params.pool.token0() == callAsset;
            if (isAmount0) {
                uint256 amount0 = LiquidityAmounts.getAmount0ForLiquidity(
                    _params.tickLower.getSqrtRatioAtTick(),
                    _params.tickUpper.getSqrtRatioAtTick(),
                    uint128(_params.amountToExercise)
                );
                ERC20(callAsset).approve(address(swapRouter), amount0);
                uint256 amountOut = swapRouter.exactInputSingle(
                    ISwapRouter.ExactInputSingleParams({
                        tokenIn: callAsset,
                        tokenOut: putAsset,
                ...
                uint256 amountReq = LiquidityAmounts.getAmount1ForLiquidity(
                    _params.tickLower.getSqrtRatioAtTick(),
                    _params.tickUpper.getSqrtRatioAtTick(),
                    uint128(_params.amountToExercise)
                );

                ERC20(putAsset).approve(address(positionManager), amountReq);

                (uint256 a0, uint256 a1, uint256 liquidity) = positionManager
                    .unusePosition(
                        PositionManager.UnusePositionParams({
                            pool: _params.pool,
                            tickLower: _params.tickLower,
                            tickUpper: _params.tickUpper,
                            liquidityToUnuse: uint128(_params.amountToExercise)
                        })
                    );

                ERC20(putAsset).transfer(msg.sender, amountOut - amountReq);
            } else {
                ...
                ERC20(callAsset).approve(address(swapRouter), amount1);
                uint256 amountOut = swapRouter.exactInputSingle(
                    ISwapRouter.ExactInputSingleParams({
                        tokenIn: callAsset,
                        tokenOut: putAsset,
                ...
                uint256 amountReq = LiquidityAmounts.getAmount0ForLiquidity(
                    _params.tickLower.getSqrtRatioAtTick(),
                    _params.tickUpper.getSqrtRatioAtTick(),
                    uint128(_params.amountToExercise)
                );
            ...
            bool isAmount0 = _params.pool.token0() == putAsset;
            if (isAmount0) {
                uint256 amount0 = LiquidityAmounts.getAmount0ForLiquidity(
                    _params.tickLower.getSqrtRatioAtTick(),
                    _params.tickUpper.getSqrtRatioAtTick(),
                ...
                ERC20(putAsset).approve(address(swapRouter), amount0);
                ...
                ERC20(callAsset).approve(address(positionManager), amountReq);
                ...
                ERC20(callAsset).transfer(msg.sender, amountOut - amountReq);
            } else {
                uint256 amount1 = LiquidityAmounts.getAmount1ForLiquidity(
                    _params.tickLower.getSqrtRatioAtTick(),
                    _params.tickUpper.getSqrtRatioAtTick(),
                ...
                ERC20(putAsset).approve(address(swapRouter), amount1);
                uint256 amountOut = swapRouter.exactInputSingle(
                    ISwapRouter.ExactInputSingleParams({
                        tokenIn: putAsset,
                        tokenOut: callAsset,
        ...
        emit ExerciseOptionRoll(
            _params.pool,
            _params.tickLower,
            _params.tickUpper,
            _params.expiry,
            _params.callOrPut,
            _params.amountToExercise,
            msg.sender
        );

        _burn(msg.sender, optionTokenId, _params.amountToExercise);
    }

    function settleOptionRoll(SettleOptionRollParams memory _params) external {
        ...
        (uint256 amount0, uint256 amount1) = LiquidityAmounts
            .getAmountsForLiquidity(
                ...
                _params.tickLower.getSqrtRatioAtTick(),
                _params.tickUpper.getSqrtRatioAtTick(),
                uint128(_params.amountToSettle)
            );
        ...
        if (_params.callOrPut) {
            bool isAmount0 = _params.pool.token0() == callAsset;
            if (isAmount0) {
                if (amount0 > 0 && amount1 > 0) {} else if (amount1 > 0) {
                    uint256 amountToSwap = LiquidityAmounts
                    ...
                    ERC20(callAsset).approve(address(swapRouter), amountToSwap);
                    uint256 amountOut = swapRouter.exactInputSingle(
                        ISwapRouter.ExactInputSingleParams({
                            tokenIn: callAsset,
                            tokenOut: putAsset,
                    ...
                    (
                        ...
                        uint256 liquidity
                            ...
                            PositionManager.UnusePositionParams({
                                ...
                                tickLower: _params.tickLower,
                                tickUpper: _params.tickUpper,
                                liquidityToUnuse: uint128(
                                    _params.amountToSettle
                                )
                            })
                        );
                    ERC20(putAsset).transfer(msg.sender, amountOut - a1);
                } else {
                    ERC20(callAsset).approve(address(positionManager), amount0);
                    (, , uint256 liquidity) = positionManager.unusePosition(
                        PositionManager.UnusePositionParams({
                            pool: _params.pool,
                            tickLower: _params.tickLower,
                            tickUpper: _params.tickUpper,
                            liquidityToUnuse: uint128(_params.amountToSettle)
                        })
                    );
                }
            } else {
                if (amount0 > 0 && amount1 > 0) {} else if (amount0 > 0) {
                    uint256 amountToSwap = LiquidityAmounts
                    ...
                    ERC20(putAsset).transfer(msg.sender, amountOut - a0);
                } else {
                    ERC20(callAsset).approve(address(positionManager), amount1);
                    (, , uint256 liquidity) = positionManager.unusePosition(
                        PositionManager.UnusePositionParams({
                            pool: _params.pool,
                            tickLower: _params.tickLower,
            ...
            if (isAmount0) {
                if (amount0 > 0 && amount1 > 0) {} else if (amount1 > 0) {
                    ...
                    ERC20(putAsset).approve(address(swapRouter), amountToSwap);
                    uint256 amountOut = swapRouter.exactInputSingle(
                        ISwapRouter.ExactInputSingleParams({
                            tokenIn: putAsset,
                            tokenOut: callAsset,
                    ...
                    ERC20(callAsset).transfer(msg.sender, amountOut - a1);
                } else {
                    ERC20(putAsset).approve(address(positionManager), amount0);
                    (, , uint256 liquidity) = positionManager.unusePosition(
                        PositionManager.UnusePositionParams({
                            pool: _params.pool,
                            tickLower: _params.tickLower,
                ...
                if (amount0 > 0 && amount1 > 0) {} else if (amount0 > 0) {
                    ...
                    ERC20(callAsset).transfer(msg.sender, amountOut - a0);
                } else {
                    ERC20(putAsset).approve(address(positionManager), amount1);
                    (, , uint256 liquidity) = positionManager.unusePosition(
                        PositionManager.UnusePositionParams({
                            pool: _params.pool,
                            tickLower: _params.tickLower,
    ...
    function _getPremiumAmount(
        ...
        if (useMinPremium) return MIN_PREMIUM;
        ...
        if (isPut) {
            return (premiumInQuote * (10 ** ERC20(putAsset).decimals())) / 1e18;
        }
        return
