...
import {PancakeV3TestLib} from "./pancake-v3-utils/PancakeV3TestLib.sol";
...
contract positionManagerHarnessTest is Test {
    ...
    function setUp() public {
        ...
        pool = IPancakeV3Pool(
            pancakeV3TestLib.deployPancakeV3PoolAndInitializePrice(
                ETH,
                LUSD,
                fee,
                initSqrtPriceX96
        ...
        positionManagerHarness = new PancakeV3SingleTickLiquidityHarnessV2(
            pancakeV3TestLib,
            positionManager,
            pcsV3Handler
        );
    ...
    function testBurnPosition() public {
        ...
        uint256 bobBalance = pcsV3Handler.balanceOf(
            bob,
            positionManagerHarness.getTokenId(pool, hook, tickLower, tickUpper)
        );

        uint256 jasonBalance = pcsV3Handler.balanceOf(
            jason,
            positionManagerHarness.getTokenId(pool, hook, tickLower, tickUpper)
        );
    ...
    function testDonation() public {
        ...
        uint256 rogerBalance = pcsV3Handler.balanceOf(
            roger,
            positionManagerHarness.getTokenId(pool, hook, tickLower, tickUpper)
        );

        uint256 tangoBalance = pcsV3Handler.balanceOf(
            tango,
            positionManagerHarness.getTokenId(pool, hook, tickLower, tickUpper)
        );
    ...
    function testPutOptionSim() public {
        ...
        PancakeV3SingleTickLiquidityHandlerV2.TokenIdInfo
            memory tki = pcsV3Handler.getTokenIdData(
                uint256(
                    keccak256(
                        abi.encode(
                            address(pcsV3Handler),
