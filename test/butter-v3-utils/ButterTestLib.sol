...
import {ButterLiquidityManagement} from "../butter-v3-utils/ButterLiquidityManagement.sol";
import {ERC20Mock} from "../mocks/ERC20Mock.sol";

contract ButterTestLib is Test {
    IButterFactory public immutable factory;
    ButterLiquidityManagement public immutable butterLiquidityManagement;
    ...
    constructor() {
        factory = IButterFactory(0xEECa0a86431A7B42ca2Ee5F479832c3D4a4c2644);
        butterLiquidityManagement = new ButterLiquidityManagement(address(factory));
        swapRouter = ISwapRouter(0xAe5c0a73bAE513fe5842DbcB8E32fc7E3e30DA57);
    }

    function deployButterPoolAndInitializePrice(address token0, address token1, uint24 fee, uint160 sqrtPriceX96)
        public
    ...
    function getCurrentSqrtPriceX96(IButterPool pool) public view returns (uint160 sqrtPriceX96) {
        (sqrtPriceX96,,,,,,) = pool.slot0();
    }

    function getCurrentTick(IButterPool pool) public view returns (int24 tick) {
        (, tick,,,,,) = pool.slot0();
    }
    ...
    function addLiquidity(AddLiquidityStruct memory _params) public returns (uint128 liquidity) {
        ...
        token0.approve(address(butterLiquidityManagement), type(uint256).max);
        token1.approve(address(butterLiquidityManagement), type(uint256).max);

        (liquidity,,,) = butterLiquidityManagement.addLiquidity(
            ButterLiquidityManagement.AddLiquidityParams({
