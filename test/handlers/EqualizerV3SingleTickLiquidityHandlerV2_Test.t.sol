...
contract EqualizerSingleTickLiquidityHandlerV2_Test is Test {
    ...
    address WFTM = 0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83; // token1
    address USDC = 0x04068DA6C83AFCFA0e13ba15A6696662335D5B75; // token0
    ...
    uint160 initSqrtPriceX96 = 1771845812700903892492222464; // 1 FTM = 2000 USDC
    ...
    function setUp() public {
        ...
        if (WFTM < USDC) {
            token0 = ERC20Mock(WFTM);
            token1 = ERC20Mock(USDC);
        } else {
            token0 = ERC20Mock(USDC);
            token1 = ERC20Mock(WFTM);
        }

        equalizerV3TestLib = new EqualizerV3TestLib();
        pool = IEqualizerV3Pool(
            equalizerV3TestLib.deployEqualizerV3PoolAndInitializePrice(WFTM, USDC, fee, initSqrtPriceX96)
        );
        ...
        payable(jason).transfer(0.1 ether);
        payable(trader).transfer(0.1 ether);
        payable(garbage).transfer(0.1 ether);
        payable(roger).transfer(0.1 ether);
        payable(tango).transfer(0.1 ether);

        console.log(alice.balance, address(this).balance);
        ...
        equalizerV3Handler = new EqualizerV3SingleTickLiquidityHandlerV2(
            ...
            0x01795efa243dc58f09de9b7c1fa74e72352806f279d62f49ef19e11c42a98292,
            address(equalizerV3TestLib.swapRouter())
        );

        positionManagerHarness = new EqualizerV3SingleTickLiquidityHarnessV2(equalizerV3TestLib, positionManager, equalizerV3Handler);
