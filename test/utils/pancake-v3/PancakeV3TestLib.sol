...
contract PancakeV3TestLib is Test {
    ...
    constructor() {
        ...
        pancakeV3LiquidityManagement = new PancakeV3LiquidityManagement(0x41ff9AA7e16B8B1a8a8dc4f0eFacd93D02d071c9);

        swapRouter = ISwapRouter(0x1b81D678ffb9C0263b24A97847620C99d213eB14);
    }

    function deployPancakeV3PoolAndInitializePrice(address token0, address token1, uint24 fee, uint160 sqrtPriceX96)
        public
    ...
    function getCurrentSqrtPriceX96(IPancakeV3Pool pool) public view returns (uint160 sqrtPriceX96) {
    ...
    function addLiquidity(AddLiquidityStruct memory _params) public returns (uint128 liquidity) {
        ...
        token0.approve(address(pancakeV3LiquidityManagement), type(uint256).max);
        token1.approve(address(pancakeV3LiquidityManagement), type(uint256).max);
