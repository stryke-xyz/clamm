...
contract SushiV3TestLib is Test {
    IUniswapV3Factory public immutable factory;
    SushiV3LiquidityManagement public immutable sushiV3LiquidityManagement;
    ...
    constructor() {
        factory = IUniswapV3Factory(0x1af415a1EbA07a4986a52B6f2e7dE7003D82231e);
        sushiV3LiquidityManagement = new SushiV3LiquidityManagement(address(factory));
        swapRouter = ISwapRouter(0x8A21F6768C1f8075791D08546Dadf6daA0bE820c);
    }

    function deploySushiV3PoolAndInitializePrice(address token0, address token1, uint24 fee, uint160 sqrtPriceX96)
        public
    ...
    function addLiquidity(AddLiquidityStruct memory _params) public returns (uint128 liquidity) {
        ...
        token0.approve(address(sushiV3LiquidityManagement), type(uint256).max);
        token1.approve(address(sushiV3LiquidityManagement), type(uint256).max);

        (liquidity,,,) = sushiV3LiquidityManagement.addLiquidity(
