...
import {UniswapV3FactoryDeployer} from "./UniswapV3FactoryDeployer.sol";
...
contract UniswapV3TestLib is Test, UniswapV3FactoryDeployer {
    ...
    function deployUniswapV3PoolAndInitializePrice(address token0, address token1, uint24 fee, uint160 sqrtPriceX96)
        public
    ...
    function addLiquidity(AddLiquidityStruct memory _params) public returns (uint128 liquidity) {
        ...
        token0.approve(address(uniswapV3LiquidityManagement), type(uint256).max);
        token1.approve(address(uniswapV3LiquidityManagement), type(uint256).max);
