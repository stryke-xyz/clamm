...
contract ButterTest is Test {
    ERC20Mock token0; // LUSD
    ERC20Mock token1; // ETH

    IButterFactory factory;
    IButterPool pool;
    uint24 fee;

    ButterLiquidityManagement butterLiquidityManagement;
    ...
    function setUp() public {
        ...
        fee = 500;
        pool = IButterPool(factory.createPool(address(token0), address(token1), fee));
    ...
    function testAddingLiquidity() public {
        ...
        (uint128 liquidity,,,) = butterLiquidityManagement.addLiquidity(
