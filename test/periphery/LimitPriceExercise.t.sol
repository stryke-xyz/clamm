...
contract LimitExerciseTest is Test {
    ...
    function testVerifySignature() public {
        ...
        bytes32 digest0 = limitExercise.computeDigest(LimitExercise.Order(1, 2, 3, address(0), address(1), address(1)));
        bytes32 digest1 = limitExercise.computeDigest(LimitExercise.Order(2, 3, 4, address(0), address(2), address(1)));
        ...
        assertEq(
            limitExercise.verify(
                alice,
                LimitExercise.Order(1, 2, 3, address(1), address(1), address(1)),
    ...
    function testFailOptionIdsMisMatch() public {
        ...
        bytes32 digest = limitExercise.computeDigest(LimitExercise.Order(3, 0, 0, address(1), address(0), address(0)));
    ...
    function testLimitExerciseCall() public {
        ...
        LimitExercise.Order memory order =
            LimitExercise.Order(1, 5e18, block.timestamp + 20 minutes, address(token0), address(optionMarket), trader);
    ...
    function testFuzzMinProfit(uint256 _minProfit) public {
        ...
        LimitExercise.Order memory order = LimitExercise.Order(
            1, _minProfit, block.timestamp + 20 minutes, address(token1), address(optionMarket), trader
        );

        LimitExercise.SignatureMeta memory sigMeta;

        (uint8 v, bytes32 r, bytes32 s) = vm.sign(privateKey, limitExercise.computeDigest(order));
    ...
    function testLimitExercisePut() public {
        ...
        LimitExercise.Order memory order = LimitExercise.Order(
            1, minProfit, block.timestamp + 20 minutes, address(token1), address(optionMarket), trader
        );
    ...
    function testCancelLimitExerciseOrder() public {
        ...
        LimitExercise.Order memory order = LimitExercise.Order(
            1, 0.05 ether, block.timestamp + 20 minutes, address(token1), address(optionMarket), trader
        );
