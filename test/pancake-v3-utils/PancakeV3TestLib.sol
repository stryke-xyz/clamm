...
import {PancakeV3LiquidityManagement} from "./PancakeV3LiquidityManagement.sol";
import {ERC20Mock} from "../mocks/ERC20Mock.sol";

contract PancakeV3TestLib is Test {
    IPancakeV3Factory public immutable factory;
    PancakeV3LiquidityManagement public immutable pancakeV3LiquidityManagement;
    ...
    constructor() {
        factory = IPancakeV3Factory(0x0BFbCF9fa4f9C56B0F40a671Ad40E0805A091865);

        pancakeV3LiquidityManagement = new PancakeV3LiquidityManagement(
            0x41ff9AA7e16B8B1a8a8dc4f0eFacd93D02d071c9
        );

        swapRouter = ISwapRouter(0x1b81D678ffb9C0263b24A97847620C99d213eB14);
    }

    function deployPancakeV3PoolAndInitializePrice(
        address token0,
        address token1,
        uint24 fee,
    ...
    function addLiquidity(
        AddLiquidityStruct memory _params
        ...
        uint160 lower = TickMath.getSqrtRatioAtTick(
            (_params.desiredTickLower / int24(10)) * 10 + 10
        );
        uint160 upper = TickMath.getSqrtRatioAtTick(
            (_params.desiredTickUpper / int24(10)) * 10
        );
        ...
        token0.approve(
            address(pancakeV3LiquidityManagement),
            type(uint256).max
        );
        token1.approve(
            address(pancakeV3LiquidityManagement),
        ...
        (liquidity, , , ) = pancakeV3LiquidityManagement.addLiquidity(
            PancakeV3LiquidityManagement.AddLiquidityParams({
    ...
    function removeLiquidity(
        RemoveLiquidityStruct memory _params
        ...
        if (_params.liquidity <= 0) {
            (_params.liquidity, , , , ) = _params.pool.positions(
                keccak256(abi.encodePacked(_params.user, tickLower, tickUpper))
            );
        }
    ...
    function performSwap(
        SwapParamsStruct memory _params
        ...
        (amountOut) = swapRouter.exactInputSingle(
            ISwapRouter.ExactInputSingleParams({
                tokenIn: _params.zeroForOne
                    ? _params.pool.token0()
                    : _params.pool.token1(),
                tokenOut: _params.zeroForOne
                    ? _params.pool.token1()
                    : _params.pool.token0(),
                fee: _params.pool.fee(),
                recipient: _params.user,
    ...
    function teleportToPrice(
        TeleportParamsStruct memory _params
    )
        public
        returns (
            uint256 amountIn,
            uint256 amountOut,
            uint160 latestSqrtPriceX96
        )
    {
        ERC20Mock token0 = ERC20Mock(_params.pool.token0());
        ERC20Mock token1 = ERC20Mock(_params.pool.token1());
