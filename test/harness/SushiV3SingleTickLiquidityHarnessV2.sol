...
contract SushiV3SingleTickLiquidityHarnessV2 is Test {
    using TickMath for int24;

    SushiV3TestLib sushiV3TestLib;
    DopexV2PositionManager positionManager;
    SushiV3SingleTickLiquidityHandlerV2 sushiV3Handler;
    IHandler handler;

    constructor(
        SushiV3TestLib _sushiV3TestLib,
        DopexV2PositionManager _positionManager,
        SushiV3SingleTickLiquidityHandlerV2 _sushiV3Handler
    ) {
        sushiV3TestLib = _sushiV3TestLib;
        positionManager = _positionManager;
        sushiV3Handler = _sushiV3Handler;
        handler = IHandler(address(_sushiV3Handler));
    }
    ...
    function mintPosition(
        ...
        (lm) = positionManager.mintPosition(
            handler,
            abi.encode(
                SushiV3SingleTickLiquidityHandlerV2.MintPositionParams({
                    pool: pool,
                    hook: hook,
                    tickLower: tickLower,
                    tickUpper: tickUpper,
    ...
    function usePosition(
        ...
        (tokens, amounts,) = positionManager.usePosition(
            handler,
            abi.encode(
                SushiV3SingleTickLiquidityHandlerV2.UsePositionParams({
                    pool: pool,
                    hook: hook,
                    tickLower: tickLower,
                    tickUpper: tickUpper,
    ...
    function unusePosition(
        ...
        positionManager.unusePosition(
            handler,
            abi.encode(
                SushiV3SingleTickLiquidityHandlerV2.UnusePositionParams({
                    pool: pool,
                    hook: hook,
                    tickLower: tickLower,
                    tickUpper: tickUpper,
    ...
    function donatePosition(
        ...
        (amounts,) = positionManager.donateToPosition(
            handler,
            abi.encode(
                SushiV3SingleTickLiquidityHandlerV2.DonateParams({
                    pool: pool,
                    hook: hook,
                    tickLower: tickLower,
                    tickUpper: tickUpper,
