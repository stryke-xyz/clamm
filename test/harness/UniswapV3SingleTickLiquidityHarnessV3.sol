...
contract UniswapV3SingleTickLiquidityHarnessV3 is Test {
    using TickMath for int24;

    UniswapV3TestLib uniswapV3TestLib;
    DopexV2PositionManagerV2 positionManager;
    UniswapV3SingleTickLiquidityHandlerV3 uniswapV3Handler;
    IHandlerV3 handler;

    constructor(
        UniswapV3TestLib _sushiV3TestLib,
        DopexV2PositionManagerV2 _positionManager,
        UniswapV3SingleTickLiquidityHandlerV3 _uniswapV3Handler
    ) {
        uniswapV3TestLib = _sushiV3TestLib;
        positionManager = _positionManager;
        uniswapV3Handler = _uniswapV3Handler;
        handler = IHandlerV3(address(_uniswapV3Handler));
    }
    ...
    function mintPosition(
        ...
        (lm) = positionManager.mintPosition(
            handler,
            abi.encode(
                UniswapV3SingleTickLiquidityHandlerV3.MintPositionParams({
                    pool: pool,
                    hook: hook,
                    tickLower: tickLower,
                    tickUpper: tickUpper,
    ...
    function usePosition(
        ...
        (tokens, amounts,) = positionManager.usePosition(
            handler,
            abi.encode(
                UniswapV3SingleTickLiquidityHandlerV3.UsePositionParams({
                    pool: pool,
                    hook: hook,
                    tickLower: tickLower,
                    tickUpper: tickUpper,
    ...
    function unusePosition(
        ...
        positionManager.unusePosition(
            handler,
            abi.encode(
                UniswapV3SingleTickLiquidityHandlerV3.UnusePositionParams({
                    pool: pool,
                    hook: hook,
                    tickLower: tickLower,
                    tickUpper: tickUpper,
    ...
    function donatePosition(
        ...
        (amounts,) = positionManager.donateToPosition(
            handler,
            abi.encode(
                UniswapV3SingleTickLiquidityHandlerV3.DonateParams({
                    pool: pool,
                    hook: hook,
                    tickLower: tickLower,
                    tickUpper: tickUpper,
